...
#add module symbol with static tokens, it is another way of specifing lex rules
for (k, v) in list(static_tokens.items ()):
    cur_mod.__dict__['t_' + v] = k

#lex rules in function form
...
class Node:
    ...
    def str_child (self, key, child, depth):
        ...
        if isinstance(child, type ([])):
            l = []
            for x in child:
              if isinstance (x, Node): l.append (x.str_depth (depth+1)) else:
...
if __name__ == '__main__':
  if (os.path.splitext(os.path.basename(sys.argv[0]))[0].lower() in ('asn2wrs', 'asn2eth')):
    print os.path.splitext(os.path.basename(sys.argv[0]))[0].lower()
    print "eth_main()"
    #eth_main()
  else:
    print os.path.splitext(os.path.basename(sys.argv[0]))[0].lower()
